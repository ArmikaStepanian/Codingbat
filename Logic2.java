...
public class Logic2 {
    ...
    public boolean closeFar(int a, int b, int c) {
        return (Math.abs(a - b) < 2 && Math.abs(a - c) > 1 && Math.abs(c - b) > 1 ||
    ...
    public int blackjack2(int a, int b) { //красивое решение, мне понравилось
        int newA = a;
        if (a > 21) newA = 0;
        int newB = b;
        if (b > 21) newB = 0;
    ...
    public boolean evenlySpaced(int a, int b, int c) {        //решение не очень, но и в готовых решениях тоже какой-то геморр
        if (a == b && b == c) return true;
        if (a != b && b != c && c != a) {
            if (Math.abs(b - a) == Math.abs(b - c))
                return true;                     //int min = Math.min(Math.min(a, b), c);  вот так искать минимум среди трех значений
    ...
    public int makeChocolate(int small, int big, int goal) {    //хорошее решение про коробку шоколада
        ...
        if (max <= big) {
            if (goal - 5 * max <= small) return (goal - 5 * max);
            else return -1;
        }
        return -1;
    }
