...
public class String2 {
    ...
    public int countHi(String str) {
        ...
        for (int i=0; i<str.length()-1; i++){
            if (str.substring(i,i+2).equals("hi")) count++;   //быстро решила
        } return count;
    }
    ...
    public boolean bobThere(String str) {
        for (int i=0; i< str.length()-2; i++){
            if (str.charAt(i)=='b' && str.charAt(i+2)=='b') return true;} //решила сходу. поиск не слова, а последовательности букв
        return false;
    }
    public boolean xyBalance(String str) {
        ...
        if (str.contains("y")) {
            ...
            for (int i = 0; i < str.length() - 1; i++) {  //костыль написанный мною лично, ниже нормальоне решение
    ...
    public String mixString(String a, String b) {
        String res = "";
        for (int i=0; i < Math.min(a.length(),b.length()); i++){  //вот этого я не знала
    ...
    public String repeatEnd(String str, int n) {
        String res = "";
        for (int i=0; i<n; i++)
            res = res + str.substring(str.length()-n); //сама решила.
        return res;
    }
    public String repeatFront(String str, int n) {
        String res = "";
        for (int i=n; i>0; i--)
            res = res + str.substring(0,i);      //сама решила. потому что знала про декремент
        return res;
    }
    public String repeatSeparator(String word, String sep, int count) {
        ...
        for (int i=0; i<count; i++)
            res = res + word + sep;
        return res.substring(0,res.lastIndexOf(sep));  //сама решила
    }
    ...
    public boolean prefixAgain(String str, int n) {
        ...
        return  (str.substring(n).contains(prefix)); //решила сама используя contains(), че городить эти костыли
    }
    ...
    public String getSandwich(String str) {
        ...
        if (f != l)
            return str.substring(f+5,l);  //сама решила
        return "";
    }
    ...
    public String zipZap(String str) {
        for (int i = 0; i < str.length()-2; i++)
            if (str.charAt(i) == 'z' && str.charAt(i+2) == 'p')
                str = str.substring(0,i+1) + str.substring(i+2);   //пол утра решала. сильно все усложняю в голове
        return str;
    }
    ...
    public String wordEnds(String str, String word) {
        ...
        while (index != -1) {                                           //дрочилась с циклом while
    ...
    public String plusOut(String str, String word) {      //Заменить все символы знаком +, а данную подстроку оставить без изменений
        ...
        int end = str.indexOf(word);                   //Используется свойство str.substring (0,0) возвращает пустой символ
    ...
    public String starOut(String str) {             //удалить звездочки и символы вокруг них
        while (str.indexOf('*') != -1) {                      //пока есть *
            ...
            else str = str.substring(0, firstStar-1) + str.substring(firstStar+2); //возвращаем сумму подстрок без звездочки и соседних символов
        }
        return str;
    }
