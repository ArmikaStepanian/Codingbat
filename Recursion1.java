...
public class Recursion1 {
    ...
    public int factorial(int n) {  //сама решила факториал, до рекурсии не додумалась
        int res = 1;
        do {
            res = res * n;
            n = n-1;
    ...
    public int fibonacci(int n) {              //Сама решила числа Фибоначчи!!!
    ...
    public int bunnyEars2(int bunnies) {
        ...
        return (bunnyEars2(bunnies-2) + 5) ;    //сама решила!
    }

    public int triangle(int rows) {   //сама решила!!!
        if (rows==0)return 0;
        return triangle(rows-1)+rows;

    }

    public int sumDigits(int n) {              //сама решила!!!
        if (n/10==0) return n;               //эта строка означает что n состоит из одной цифры
        return sumDigits(n/10) + n%10;
    }
    public int powerN(int base, int n) {  //сама решила. По наитию!
        if(n==0) return 1;
