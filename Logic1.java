...
public class Logic1 {
    ...
    public boolean cigarParty1(int cigars, boolean isWeekend) {
        if (isWeekend){
            return cigars>=40;}                   //Типовое решение - когда два булева условия и два числовых условия
    ...
    public boolean squirrelPlay(int temp, boolean isSummer) {
        if (isSummer){
            return (temp>=60 && temp<=100);}     //Типовое решение - когда два булева условия и два числовых условия
    ...
    public int caughtSpeeding(int speed, boolean isBirthday) {
        if (isBirthday) speed = speed-5;                   //логика в том, что нужно уменьшать сущ. скорость а не повышать лимиты
    ...
    public String alarmClock(int day, boolean vacation) {
        if (vacation){              //!!!!Первое условие самое однозначное. И в дальнейшем не имеет значения.
            if(day>=1 && day<=5)
                return "10:00";
                return "off";}
        if(day>=1 && day<=5)     //я не знала что так можно записывать
                return "7:00";
                return "10:00";
        }
    public boolean in1To10(int n, boolean outsideMode) {   //внимательно с И и с ИЛИ,  интересный синтаксис
        if (outsideMode){                   //сначала ставим булево условие//!!!!Первое условие самое однозначное. И в дальнейшем не имеет значения.
            return (n<=1 || n>=10);}       //затем проверяем числовое условие,
        return  (n>=1 && n<=10);          // затем проверяем другое булево значение и числовое значение
    }
    public boolean answerCell(boolean isMorning, boolean isMom, boolean isAsleep) {
        if (isAsleep){return false;}
        if (isMom){return true;}             //!!!!Первое условие самое однозначное. И в дальнейшем не имеет значения.
        return  (!isMorning);

    }
    public int teaParty(int tea, int candy) {
        if (tea<5||candy<5) return 0;       //важен порядок исключения!!!!!!!!!!!!!!!!!   меньше пяти - пошли вон
        if (tea>=candy*2 || candy>=tea*2) return 2;             // двойные значения, а переменные больше 5 по-любому
    ...
    public boolean twoAsOne(int a, int b, int c) {
        int sum = a+b+c;
        return (sum%2==0 && (sum/2==a || sum/2==b || sum/2==c)); //тут я чтото загнула с логикой
    }
